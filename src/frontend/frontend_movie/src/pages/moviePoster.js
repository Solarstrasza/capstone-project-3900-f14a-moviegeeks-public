...
function MoviePoster({title, id, uid}) {
    ...
    if (movies[0].title == 'Your Name') {
        return (
            ...
            >
            <div class = "img__wrap">
            <Link to={`/fetch_movie_detail/${user.uid}/${id}`} onClick={() => changeLocation(`/fetch_movie_detail/${user.uid}/${id}`)} ><img src = "https://image.tmdb.org/t/p/w500//q719jXXEzOoYaps6babgKnONONX.jpg" style = {{maxHeight: 420}}></img>
            {movieInfo.rating != 0 && <div class="img__description_layer_rating">
            ...
            <div style = {{
                ...
                }} class="img__description_layer_title">
                <h3>{title}</h3>
            </div>
            </Link>
    ...
    if (movies[0].title == 'Kate') {
        return (
            ...
            >
            <div class = "img__wrap">
            <Link to={`/fetch_movie_detail/${user.uid}/${id}`} onClick={() => changeLocation(`/fetch_movie_detail/${user.uid}/${id}`)} ><img src = "https://image.tmdb.org/t/p/w500//mUvikzKJJSg9khrVdxK8kg3TMHA.jpg" style = {{maxHeight: 420}}></img>
            {movieInfo.rating != 0 && <div class="img__description_layer_rating">
                <Rating style = {{color : "#FFD700", padding: 10}} value={Math.round(movieInfo.rating)} readOnly precision={0.1} size="large"emptyIcon={<StarIcon style={{ color: "#B0ADAC" }} fontSize="inherit" />}/>
                <h3>{Math.round(movieInfo.rating)}</h3> 
            </div>}
    ...
    if (!movies[0].poster_path) {
        return (
            ...
            <div class="img__description_layer_title">
                <h3>{title}</h3>
            </div>
            </Link>
